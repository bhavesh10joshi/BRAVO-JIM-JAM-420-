...
async function getdata(rnum) {
  const response = await fetch(`https://cors-anywhere.herokuapp.com/https://osdr.nasa.gov/osdr/data/osd/meta/${rnum}?api_key=ncagcf9S1cVQGAWZs3Ht36YafeR9cHZimo9pKTym`);
  const data = await response.json();
  return data;
}

butt.addEventListener("click", async () => {
  const rnum = inp.value.slice(4);
  console.log(rnum);
  const data = await getdata(rnum);
  console.log(data);
  head1.innerText = `OSD-${rnum}`;
  head2.innerText = data.study[`OSD-${rnum}`].studies[0].title;
  prh.innerText = "Description";
  prt.innerText = data.study[`OSD-${rnum}`].studies[0].description;
    ...
    lele.item(i).addEventListener("click", () => {
      ...
      switch (i + 1) {
        case 1:
          prh.innerText = "";
          prt.innerText = "";
          const std = data.study[`OSD-${rnum}`].studies[0];
        ...
        case 3:
          ...
          mqc.innerHTML = "";
          const s = data.study[`OSD-${rnum}`].studies[0];
        ...
        case 4:
          ...
          mqc.innerHTML = `<iframe src="https://osd-${rnum}.netlify.app/" width="800" height="600"></iframe>`;
          break;
